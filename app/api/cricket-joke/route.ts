...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${API_CONFIG.baseURL}/cricket-joke`, {
      method: "GET",
    ...
    return NextResponse.json({
      joke:
        data.joke ||
        data.response ||
        "Why don't cricket players ever get cold? Because they're always close to the wicket!",
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
    console.error("Cricket Joke API Error:", error)
    return NextResponse.json(
      {
        error: "Unable to fetch cricket joke at the moment.",
      },
      { status: 500 },
    )
  }
